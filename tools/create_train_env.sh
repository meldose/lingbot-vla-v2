...
usage() {
  cat <<'USAGE'
Usage: bash tools/create_train_env.sh [--env-name NAME] [--recreate] [--resume] [--cpu] [--flash-attn-wheel PATH]
...
Use --cpu for development or CPU inference environments. This skips CUDA
validation and FlashAttention, neither of which is available without an NVIDIA GPU.
USAGE
}
...
if sys.argv[1] != "1":
    assert torch.cuda.is_available(), "torch CUDA is not available (pass --cpu for a CPU-only environment)"
PY
}
...
if [[ "${CPU_ONLY}" == "1" ]]; then
  echo "Skipping FlashAttention in CPU-only mode."
